defmodule Semanteq.Providers.Anthropic do
  @moduledoc """
  Anthropic Claude API provider for G-Lisp code generation.

  Implements the `Semanteq.Provider` behaviour for the Anthropic Claude API.
  This provider uses Claude to generate, validate, and refine G-expressions
  ...
  def health_check do
    ...
    if cfg[:api_key] && cfg[:api_key] != "" do
      {:ok, %{status: "configured", model: cfg[:model], provider: :anthropic}}
    else
